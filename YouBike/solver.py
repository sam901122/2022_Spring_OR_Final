...
def solve(c_loc, c_weight, t_loc, t_weight, dist):
    ...
    c_sum = sum(c_weight)
    t_sum = sum(t_weight)
    c_weight = [w / c_sum for w in c_weight]
    t_weight = [w / t_sum for w in t_weight]
    ppc = c_sum / len(c_weight) / len(t_weight)
    ...
    d = [[distance(centers[k][0], centers[k][1], t_loc[j][0], t_loc[j][1]) for j in T] for k in S]
    ...
    model.setObjective(w1 * (sum(c) / len(c_loc)) + w2 * (m / (dist * 10000)) + w3 * (u / len(c_loc)), gp.GRB.MINIMIZE)
    # model.setObjective(sum(c), gp.GRB.MINIMIZE)
    model.optimize()
