...
def solve( c_loc, c_weight, t_loc, t_weight, dist, w=[ 1, 1, 1 ] ):
    ...
    def rotate( locations, theta ):
        return [ [ cos( theta ) * x - sin( theta ) * y,
                   sin( theta ) * x + cos( theta ) * y ] for [ x, y ] in locations ]
    ...
    model.setObjective(
        w1 * ( sum( c ) / len( c_loc ) ) + w2 * ( sum( r ) / ( len( t_loc ) * dist * 100 ) ) + w3 *
        ( u / len( c_loc ) ),
        gp.GRB.MINIMIZE,
    )
